...
//	protected Map<String, Map<String, Vector<Termin>>> besitzerTermine;
	...
	protected SimpleServer()
	{
		...
		try
		{
			...
			delete(0);									 // 1. Testtermin gel�scht
			System.out.println(getTermin(1).toString()); // 2. Testtermin ausgegeben
		}
		catch (Exception e)
		{
			e.printStackTrace();
...
//		delete(termin, termin.getBesitzer(), besitzerTermine);
	...
	{
		//throw new TerminException("Not yet implemented!");
		Termin returnTermin = null;
		for (String s : teilnehmerTermine.keySet()) // bis jetzt nur ADM
		{
			for (Vector<Termin> y : teilnehmerTermine.get(s).values()) // Key -> CurrentDate | Value Vector<Termine>
			{	
				for (Termin termin : y) 
				{
